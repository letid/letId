...
namespace app\map
{
  ...
  class definition extends mapController
  {
    ...
    private function home_definition($d,$l,$p)
    {
      ...
      foreach($d as $w => $v):
        if($k=key($v) and $w==$k):
          $definition['definition.word'][]=array('w'=>$w,'l'=>$l,'p'=>$p,'d'=>app\dictionary::requestHtmlEngine($v[$k]));
        else:
          foreach($v as $word => $trans)$row[] = array('w'=>$word,'l'=>'en','p'=>$p,'d'=>app\dictionary::requestHtmlEngine($trans));
          $definition['definition.sentence'][]=array('w'=>$w,'p'=>$p,'l'=>$l,'definition.word'=>$row);
        endif;
      endforeach;
      return array(
        'layout'=>array(
          'Title'=>'{q} in Myanmar definition.',
