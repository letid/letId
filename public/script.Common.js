...
var fn={
	...
	data:{
		link:function(e){
			$.each(e,function(k,v){fn[v]=$('link[rel="*"]'.replace('*',v)).attr('href');});
		},
		meta:function(e){
			$.each(e,function(k,v){fn[v]=$(fn.name.meta.replace('*',v)).attr('content');});
		}
	},
	// ra:function(e,r){
	// 	return e.join(r||'');
	...
	store:{
		s:function(e,v,d){
			...
			document.cookie = escape(e) + "=" + escape(v) + expires + "; path=/";
		},
		g:function(e){
			var esc = escape(e) + "=";
